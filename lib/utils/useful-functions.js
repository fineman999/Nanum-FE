...
export const isExistToken = () => {
  ...
  export const sseConnect = (id) =>{
    const eventSource = new EventSource(`http://20.214.170.222:8000/web-flux-service/api/v1/alerts/user?param=${id}`); //구독
    return eventSource;
  }
  ...
  export const displayedASpringMVC = (value) =>{
    ...
    const now_date = new Intl.DateTimeFormat("ko", { dateStyle: 'long' }).format(today);
    const utc_date = new Intl.DateTimeFormat("ko", { dateStyle: 'long' }).format(timeValue);
    const utc_Intl = new Intl.DateTimeFormat("ko", { timeStyle: 'short' }).format(timeValue);
  ...
  export const displayedAt = (value) =>{
    ...
    const now_date = new Intl.DateTimeFormat("ko", { dateStyle: 'long' }).format(today);
    const utc_date = new Intl.DateTimeFormat("ko", { dateStyle: 'long' }).format(utc);
    
    if(now_date !== utc_date){
      return utc_date.split("년 ")[1];
    }
    const utc_Intl = new Intl.DateTimeFormat("ko", { timeStyle: 'short' }).format(utc);
