...
export const baseURL = "http://20.214.170.222:8000/user-service/api/v1";
//로그인 baseurl
export const baseURL2 = "http://20.214.170.222:8000/user-service";
...
export const putPassword = async ({ userId, newPw }) => {
  const res = await Api.put(`/users/${userId}/pw`, { newPw: newPw });
  return res;
};

//기존 번호와 일치한지 확인
export const getOriginPw = async ({ userId, pwd }) => {
  const res = await Api.get(`/check/${userId}/pwd/${pwd}`);
  return res;
};
