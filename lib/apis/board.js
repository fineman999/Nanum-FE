...
export const deleteReplyNest = async (data) => {
  try {
    const result = await Api.delete(
...
export const getPostsByUser = async ({ userId, cancelToken }) => {
  try {
    const result = await Api.getCancelToken(
      `https://nanum.site/board-service/api/v1/posts/users/`,
      `${userId}?&sort=createAt,desc`,
      cancelToken
    );
    if (!result) {
      throw new Error(`${result} not allowd`);
